...
export class ModuloComponent { 
  ...
  openModalManageMenus(modulo: ModuloData): void {
    // Load menus for this module before opening modal
    this.menusService.getMenus({ id_modulo: modulo.id, page: 1, page_size: 100 }).subscribe({
      next: (response) => {
        ...
        const dialogRef = this.dialog.open(MenuModalComponent, {
          width: '50rem',
          height: 'auto',
          data: { modulo: modulo, menus: mappedMenus },
          disableClose: true
        });
